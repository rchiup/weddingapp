    ...
    try:
        ...
        for collection_name in ("singles_likes", "singles_passes"):
            for snapshot in _event_collection(event_id, collection_name).stream():
                ...
                if item.get("actorId") == viewer_id:
                    seen.add((item.get("targetId") or "").strip())
        users = []
        for snapshot in _event_collection(event_id, "singles").stream():
            if snapshot.id == viewer_id or snapshot.id in seen:
                continue
            item = snapshot.to_dict() or {}
            users.append({"userId": snapshot.id, "name": (item.get("name") or "Invitado").strip(), "activatedAt": item.get("activatedAt") or ""})
...
def like_user(event_id):
    ...
    user_id = (data.get("userId") or "").strip(); target_id = (data.get("targetUserId") or "").strip()
    validation_error = _require_pair(event_id, user_id, target_id)
    if validation_error:
        return jsonify({"error": validation_error}), 400
    try:
        likes = _event_collection(event_id, "singles_likes"); now = _now_iso()
        likes.document(_action_id(user_id, target_id)).set({"actorId": user_id, "targetId": target_id, "createdAt": now}, merge=True)
        ...
        if reciprocal:
            ...
            _event_collection(event_id, "singles_dm").document(match_id).set({**payload, "matchedAt": now, "lastMessage": "", "lastMessageAt": "", "lastSenderId": "", "lastReadAt": {}}, merge=True)
        return jsonify({"liked": True, "matched": reciprocal, "matchId": match_id if reciprocal else ""}), 200
    except Exception as error:
        return jsonify({"error": str(error)}), 500
...
def pass_user(event_id):
    ...
    try:
        now = _now_iso()
        _event_collection(event_id, "singles_passes").document(_action_id(user_id, target_id)).set({"actorId": user_id, "targetId": target_id, "createdAt": now}, merge=True)
...
def get_matches(event_id):
    ...
    try:
        ...
        for snapshot in _event_collection(event_id, "singles_matches").stream():
            item = snapshot.to_dict() or {}; participant_ids = [str(value) for value in item.get("participantIds") or []]
            if viewer_id not in participant_ids:
                continue
            other_id = next((value for value in participant_ids if value != viewer_id), ""); names = item.get("participantNames") or {}
            matches.append({"matchId": snapshot.id, "otherUserId": other_id, "otherName": names.get(other_id) or "Invitado" if isinstance(names, dict) else "Invitado", "matchedAt": item.get("matchedAt") or ""})
...
def reset_qa_social_state(event_id):
    ...
    try:
        ...
        for name in ("singles_likes", "singles_passes"):
            deleted["actions"] += _delete_collection_documents(_event_collection(event_id, name), lambda _, item: item.get("actorId") in qa_ids or item.get("targetId") in qa_ids)
        deleted["matches"] += _delete_collection_documents(_event_collection(event_id, "singles_matches"), lambda _, item: bool(qa_ids.intersection(str(value) for value in item.get("participantIds") or [])))
        for thread in list(_event_collection(event_id, "singles_dm").stream()):
            item = thread.to_dict() or {}; participants = {str(value) for value in item.get("participantIds") or []}
