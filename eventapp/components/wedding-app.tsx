...
function Join() {
  const { join } = useApp(); const router = useRouter(); const [name, setName] = useState(""); const [code, setCode] = useState(""); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  const submit = async (e: FormEvent) => {
    e.preventDefault();
    const normalized = code.trim().toUpperCase();
    if (normalized.length < 4) return setError("Ingresa un código válido");
    if (!normalized.endsWith("-NOVIOS") && name.trim().length < 2) return setError("Ingresa tu nombre");
    setBusy(true); setError("");
    try { await join(name, normalized); router.replace("/entry"); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "No pudimos abrir el evento."); }
    finally { setBusy(false); }
  };
  const pasteCode = async () => {
    try { setCode((await navigator.clipboard.readText()).trim().toUpperCase()); }
    catch { setError("El navegador no permitió pegar. Mantén presionado el campo y elige Pegar."); }
  };
  return <main className="join"><div className="join-inner"><div className="heart">♡</div><h1>Wedding App</h1><p>Únete al evento de alguien especial</p><form className="card form" onSubmit={submit}><label>Tu nombre<input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej: María García" autoComplete="name" disabled={busy} /></label><label>Código del evento<div className="input-row"><input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="Ej: CAROYNONI" autoCapitalize="characters" autoCorrect="off" disabled={busy} /><button type="button" className="paste" onClick={pasteCode} disabled={busy}>Pegar</button></div></label><small>El código te lo dieron los novios 💍</small>{error && <Notice error>{error}</Notice>}<button className="primary" disabled={busy}>{busy ? "Abriendo evento…" : "Unirme al evento"}</button></form><small>Demo: CAROYNONI (invitado) · CAROYNONI-NOVIOS (admin)</small><Link className="text-link" href="/create">Crea tu propio evento</Link></div></main>;
}
function Home() {
  const { session, leave } = useApp(); const router = useRouter();
  const items = menu.filter((item) => {
    const href = item[1];
    if (href === "/solteros/chats") return session.settings.singlesEnabled && session.isSingle;
    if (href === "/fotos") return session.settings.photosEnabled;
    if (href === "/mesas") return session.settings.tablesVisible;
    if (href === "/lista_novios") return session.settings.giftRegistryEnabled;
    return true;
  });
  if (!session.eventId) return <Join />;
  return <main className="home"><GuestOnboarding/><section className="hero"><div className="hero-copy"><span className="eyebrow">BIENVENIDOS</span><h1>{session.eventName}</h1><p>{session.userName ? `Hola, ${session.userName}` : "Celebremos juntos"}</p><Countdown date={session.eventDate} /></div></section><section className="menu-grid">{items.map(([label, href, image]) => <Link href={href} className="menu-card" style={{ backgroundImage: `linear-gradient(0deg,rgba(0,0,0,.48),rgba(0,0,0,.12)),url(${image})` }} key={href}><span>{label}</span></Link>)}{session.isAdmin && <Link href="/novios_admin" className="menu-card" style={{ backgroundImage: "linear-gradient(0deg,rgba(0,0,0,.48),rgba(0,0,0,.12)),url(https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?auto=format&fit=crop&w=1200&q=80)" }}><span>Panel de novios</span></Link>}</section><footer><Link href="/create">Crea tu propio evento</Link><button onClick={() => { leave(); router.push("/event_join"); }}>Salir del evento</button></footer></main>;
}
function Countdown({ date }: { date: string }) { const target = date ? new Date(date).getTime() : Number.NaN; const [now, setNow] = useState(Date.now()); useEffect(() => { const id = setInterval(() => setNow(Date.now()), 60_000); return () => clearInterval(id); }, []); if (!Number.isFinite(target)) return <div className="countdown"><span>Gracias por ser parte de este momento</span></div>; const days = Math.ceil((target - now) / 86400000); if (days < 0) return <div className="countdown"><span>Gracias por celebrar con nosotros</span></div>; return <div className="countdown"><b>{days}</b><span>{days === 1 ? "día" : "días"} para celebrar</span></div>; }
...
function Directions() {
  ...
  return <Page title="Cómo llegar">{error && <Notice error>No pudimos cargar la ubicación. {error}</Notice>}<div className="list">{locations.map((location, i) => { const lat = Number(location.latitude), lng = Number(location.longitude); return <div className="card destination" key={i}><span className="pin">⌖</span><div><h2>{String(location.label || (i ? "Fiesta / recepción" : "Ceremonia"))}</h2><p>{lat.toFixed(6)}, {lng.toFixed(6)}</p><div className="destination-actions"><a className="primary button-link" target="_blank" rel="noreferrer" href={String(location.wazeUrl || `https://waze.com/ul?ll=${lat},${lng}&navigate=yes`)}>Abrir en Waze</a><a className="ghost-button button-link" target="_blank" rel="noreferrer" href={`https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`}>Google Maps</a></div></div></div>; })}{!error && !locations.length && <Notice>Cargando ubicaciones…</Notice>}</div></Page>;
}
function Checkin() {
...
function Admin() { const { session } = useApp(); const [guests, setGuests] = useState<RecordData[]>([]); const [rsvps, setRsvps] = useState<RecordData[]>([]); const [photos, setPhotos] = useState<RecordData[]>([]); const [status, setStatus] = useState(""); useEffect(() => { Promise.all([listEventCollection(session.eventId,"guests"),listEventCollection(session.eventId,"rsvps"),listEventCollection(session.eventId,"photos")]).then(([a,b,c]) => {setGuests(a);setRsvps(b);setPhotos(c)}).catch((e) => setStatus(String(e))); }, [session.eventId]); const exportCsv = () => { const rows = [["Nombre","Mesa","Estado"], ...guests.map((x) => [String(x.name||""),String(x.tableNumber||""),String(x.status||"")])]; const csv = rows.map((r) => r.map((x) => `"${x.replaceAll('"','""')}"`).join(",")).join("\n"); const a=document.createElement("a"); a.href=URL.createObjectURL(new Blob(["\ufeff"+csv],{type:"text/csv"})); a.download=`invitados-${session.eventId}.csv`; a.click(); };
  ...
  return <Page title="Panel de novios"><div className="stats"><div className="card"><b>{guests.length}</b><span>Invitados</span></div><div className="card"><b>{rsvps.filter((x)=>x.attending).length}</b><span>Confirmados</span></div><div className="card"><b>{photos.length}</b><span>Fotos</span></div></div><div className="card admin-links"><h2>Administrar evento</h2><button onClick={exportCsv}>⇩ Exportar invitados CSV</button><Link href="/mesas">▦ Organizar mesas</Link><Link href="/checkin">✓ Ver llegadas</Link><Link href="/fotos">▧ Galería y fotos</Link><Link href="/qa/social">⚗ QA Lab: matches y chat</Link></div>{status && <Notice error>{status}</Notice>}</Page>; }
